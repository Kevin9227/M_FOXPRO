...
tudo_ok = .F.
DO FORM entrada	
	&& Formulário do Registro de Usuário - windowType = modal
...
_SCREEN.CLOSABLE    = .T.
_SCREEN.CAPTION     =  usoftwarename +' HendaSoft ' + DTOC( DATE() )	 + '   Versão Demo' &&Colocar data e hora no topo do programa
_SCREEN.VISIBLE     = .T. 
_SCREEN.ICON        = SYS(5)+SYS(2003)+'\favicon.ico' 
_SCREEN.MAXBUTTON   = .T.
...
msg ("BEM VINDO AO SISTEMA HENDASOFT..."+chr(13)+"PRONTO PARA COMEÇAR ?")&&,0+32,"....BEMVINDO...")
	     && Âncora. Faz com que o programa fique esperando as ações ;
