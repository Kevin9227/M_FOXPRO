...
Scan   
	...
	if !empty(artigo.ref) and artigo.ref!="codigo" 
		...
		if  RECCOUNT("crsst") > 0      
			select crsst     
			** Actualização dos dados principais da ficha do artigo     
...
*			replace st.familia   with alltrim(artigo.grupo)+alltrim(artigo.familia)     			
			...
			if !TABLEUPDATE(.t.,.t.,"ST")      
				Tablerevert(.t.,"ST")      
				Messagebox("Não foi possível inserir o artigo "+alltrim(upper(artigo.ref)))      
				return     
*!*				else      
*!*	     					
*!*						Messagebox("Não foi possível inserir as observações do artigo "+alltrim(upper(artigo.ref)))       								
